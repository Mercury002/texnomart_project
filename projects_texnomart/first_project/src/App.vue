  ...
  <div>
    <div class="main d-flex flex-column justify-content-center align-items-center">
      ...
      <div class="main__form">
        <Datepicker @mainDate="mainDate" />
        <div class="input-group mb-3 mt-1">
          <input type="text" class="form-control" placeholder="Чем займемся ?" v-model="taskText">
          <button :disabled="!(taskText && date)" class="btn btn-outline-secondary" type="button"
            @click="addNewTask">Прикрепить</button>
        </div>
        <div class="main__tasks rounded">
          <ul v-if="tasks.length">
            <li v-for="task in tasks" class="main__tasks--task d-flex align-items-center justify-content-between"
              :key="task.id">
              <div class="d-flex justify-content-center align-items-center">
                <input class="form-check-input mt-0" type="checkbox" value="" :checked="task.isDeleted"
                  @click="checkInput(task)" id="flexCheckDefault">

                <p class="mx-2 mb-0" :class="[task.isDeleted ? 'text-decoration-line-through' : '']">{{
                  getFormattedDate(task.date)
                }}</p>
                <strong :class="[task.isDeleted ? 'text-decoration-line-through' : '']">{{
                  task.task
                }}</strong>
              </div>
              <span class="main__tasks--trash" @click="deleteTask(task.id)">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000"
                  version="1.1" id="Capa_1" width="25px" height="25px" viewBox="0 0 470.713 470.714"
                  xml:space="preserve">
                  <g>
                    <g>
                      <path
...
export default {
  ...
  methods: {
    ...
    // Formatting date from component
    ...
    // PUT method or method for input check/unCheck
    ...
    async getAllData() {
      await fetch('http://localhost:3000/tasks/').then(res => res.json()).then(data => this.tasks = data)
    },
    // Delete method
    async deleteTask(id) {
